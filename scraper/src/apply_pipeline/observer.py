...
BUTTON_TAGS = {"button"}
ERROR_RE = re.compile(r"\b(error|required|invalid|captcha|sign in|login|expired|not found)\b", re.IGNORECASE)
BLOCKER_RE = re.compile(r"\b(captcha|sign in|login|job no longer|expired|not found|assessment)\b", re.IGNORECASE)
...
class _ApplyFormParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag == "button" or (tag == "input" and (attrs_dict.get("type") or "").lower() in {"button", "submit"}):
            ...
            self.current_button = {
                "id": element_id(tag, attrs_dict, len(self.buttons)),
                "text": text,
...
def observe_html(html: str, *, url: str = "about:blank") -> PageSnapshot:
    ...
    return PageSnapshot(url=url, fields=tuple(fields), buttons=buttons, errors=page_text, blockers=blockers)
...
def observe_page(page: Any) -> PageSnapshot:
    ...
    return PageSnapshot(url=str(getattr(page, "url", "about:blank")), fields=fields, buttons=buttons, errors=errors, blockers=blockers)
