...
CAPTCHA is automatic. Detect and complete it with the OMP `browser` or `computer` tool on the visible CMUX surface, re-grounding with fresh DOM and browser/screenshot state before and after the interaction. CAPTCHA alone never triggers user escalation, a `needs_user` outcome, or a blocked run. Do not drive the CAPTCHA through ad hoc browser-JS probes or unverified scripts; those bypass the visible surface the user owns and frequently target the wrong browser instance. If the OMP browser helper cannot solve the challenge, use the OMP `computer` tool on the same visible CMUX surface.
...
1. CMUX current-browser helper on the exact bound surface.
2. Pinned Playwright CLI only when it can attach to that exact same browser surface; never launch or attach a second browser session. The current CMUX WKWebView surface exposes no such attachment.
3. Freshly grounded native computer interaction on the same CMUX surface for a remaining browser/OS control, including the file chooser. Take a fresh DOM/browser snapshot and desktop screenshot first, act once, then immediately re-observe.
...
- OMP `browser` helpers on the visible CMUX browser surface own ordinary browser actions. Pinned Playwright CLI is the first fallback for a control the browser helper cannot operate; the OMP `computer` tool is the final native browser/OS fallback when available. Every path must follow the quick-start mapping, recording, re-observation, and retention rules.
...
- CAPTCHA challenges: detect and complete through the OMP `browser` or `computer` tool on the visible CMUX browser surface, never through ad hoc browser-JS probes or unverified scripts. Re-ground with fresh DOM and snapshot before and after. CAPTCHA alone must never trigger user escalation, a `needs_user` outcome, or a blocked run. Record detection, resolution method, and outcome in the private ledger.
