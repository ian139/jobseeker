...
@dataclass(frozen=True)
class ResumeProfile:
    """Fully parsed immutable structured profile v1/v2."""
    ...
    additional_experience_context: tuple[Mapping[str, Any], ...] = ()
    research: tuple[Mapping[str, Any], ...] = ()
    career: Mapping[str, Any] | None = None
    working_style: Mapping[str, Any] | None = None
    professional_narratives: Mapping[str, Any] | None = None
    interests: Mapping[str, Any] | None = None
    tools_and_workflows: Mapping[str, Any] | None = None
...
def _open_regular(path: Path, max_bytes: int, description: str) -> tuple[int, os.stat_result]:
    ...
    try:
        ...
        if initial.st_uid != os.getuid() or initial.st_mode & 0o022:
            raise ValueError(f"{description} must be owner-owned and not group- or world-writable")
        if description == "profile JSON" and initial.st_mode & 0o077:
            raise ValueError("profile JSON must be owner-private")
...
def _check_keys(obj: dict[str, Any], required: frozenset[str], allowed: frozenset[str], label: str) -> None:
    actual = set(obj)
...
def _parse_date_range(obj: Any, label: str) -> _DateRange:
    obj = _dict(obj, label)
    _check_keys(obj, frozenset({"display"}), frozenset({"start", "end", "display"}), label)
    display = _nonempty_string(obj["display"], f"{label}.display")
    start = _nonempty_string(obj["start"], f"{label}.start") if "start" in obj else display
    end = _nonempty_string(obj["end"], f"{label}.end") if "end" in obj else display
    return _DateRange(start=start, end=end, display=display)
...
def _parse_education_entry(obj: Any, label: str) -> _EducationEntry:
    obj = _dict(obj, label)
    req = frozenset({"id", "institution", "location", "degree", "dates", "graduation", "keywords", "sources"})
    allowed = frozenset({"id", "institution", "location", "degree", "dates", "graduation", "keywords", "sources", "coursework", "academic_context"})
    _check_keys(obj, req, allowed, label)
    return _EducationEntry(
        ...
        keywords=_parse_string_list(obj["keywords"], f"{label}.keywords", nonempty=True),
        sources=_parse_string_list(obj["sources"], f"{label}.sources", nonempty=True),
        coursework=_parse_string_list(obj["coursework"], f"{label}.coursework") if "coursework" in obj else (),
        academic_context=obj.get("academic_context"),
    )
...
def _parse_skill_entry(obj: Any, label: str) -> _SkillEntry:
    obj = _dict(obj, label)
    _check_keys(obj, frozenset({"name", "keywords", "sources"}), frozenset({"name", "keywords", "sources", "verification"}), label)
    return _SkillEntry(
        name=_nonempty_string(obj["name"], f"{label}.name"),
        keywords=_parse_string_list(obj["keywords"], f"{label}.keywords", nonempty=True),
        sources=_parse_string_list(obj["sources"], f"{label}.sources", nonempty=True),
...
def _parse_source(obj: Any, label: str) -> _Source:
    obj = _dict(obj, label)
    req = frozenset({"id", "type", "location", "retrieved_at", "notes"})
    allowed = frozenset({"id", "type", "location", "sha256", "retrieved_at", "notes"})
    _check_keys(obj, req, allowed, label)
    sha256 = _sha256_string(obj["sha256"], f"{label}.sha256") if "sha256" in obj else hashlib.sha256(json.dumps(obj, sort_keys=True).encode("utf-8")).hexdigest()
    return _Source(
        id=_nonempty_string(obj["id"], f"{label}.id"),
...
def _parse_profile_bytes(raw: bytes, path: Path) -> ResumeProfile:
    ...
    req = frozenset({"schema_version", "skills", "experience", "leadership", "education", "projects", "others"})
    if version == 1:
        _check_exact_keys(payload, req, "profile root")
    else:
        allowed = frozenset({
            "schema_version", "skills", "experience", "leadership", "education", "projects", "others",
            "profile_purpose", "profile_scope", "additional_experience_context", "research", "career",
            "working_style", "professional_narratives", "interests", "tools_and_workflows"
        })
        _check_keys(payload, req, allowed, "profile root")
    ...
    profile = ResumeProfile(
        ...
        projects=projects,
        others=others,
        profile_purpose=payload.get("profile_purpose"),
        profile_scope=payload.get("profile_scope"),
        additional_experience_context=tuple(payload.get("additional_experience_context", ())),
        research=tuple(payload.get("research", ())),
        career=payload.get("career"),
        working_style=payload.get("working_style"),
        professional_narratives=payload.get("professional_narratives"),
        interests=payload.get("interests"),
        tools_and_workflows=payload.get("tools_and_workflows"),
    )
...
def _ats_text_forms(value: str) -> tuple[str, str]:
    lowered = value.casefold()
    lowered = lowered.replace("c++", "cplusplus").replace("c#", "csharp")
    lowered = lowered.replace(".net", "dotnet").replace("ci/cd", "cicd")
...
def _profile_evidence(profile: ResumeProfile) -> tuple[str, ...]:
    ...
    for entry in profile.education:
        ...
        if entry.academic_context:
            def add_val(v: Any) -> None:
                if isinstance(v, str):
                    if v.strip(): values.append(v)
                elif isinstance(v, (list, tuple)):
                    for x in v: add_val(x)
                elif isinstance(v, dict):
                    for x in v.values(): add_val(x)
            add_val(entry.academic_context)
    def add_val(v: Any) -> None:
        if isinstance(v, str):
            if v.strip(): values.append(v)
        elif isinstance(v, (list, tuple)):
    ...
    for entry in profile.projects:
        ...
        for bullet in entry.bullets:
            values.extend((bullet.text, *bullet.keywords))
    if profile.additional_experience_context:
        add_val(profile.additional_experience_context)
    if profile.research:
        add_val(profile.research)
...
def _advisory_bonus(identifier: str, claim_rank: Mapping[str, int]) -> float:
    rank = claim_rank.get(identifier)
    if rank is None or rank >= 8:
        return 0.0
    return 0.75 - rank * 0.08
def _select_skills(
    profile: ResumeProfile,
    terms: Mapping[str, float],
    claim_rank: Mapping[str, int] | None = None,
) -> tuple[str, ...]:
    scored: list[tuple[float, int, str, int, int]] = []
    claim_rank = claim_rank or {}
    default_rank = len(claim_rank) + 1
    ...
    for category_index, (category, entries) in enumerate(ordered_categories):
        for entry_index, entry in enumerate(entries):
            ...
            score += _advisory_bonus(f"skill:{category}:{entry.name}", claim_rank)
            if score > 0:
                skill_id = f"skill:{category}:{entry.name}"
                scored.append((score, claim_rank.get(skill_id, default_rank), entry.name, category_index, entry_index))
    ...
    for _score, _rank, name, _category_index, _entry_index in scored:
        ...
        if len(result) >= _MAX_SKILLS:
            break
    if len(", ".join(result)) < _MIN_SKILLS_LINE_CHARS:
        fallback_entries = [
            (claim_rank.get(f"skill:{category}:{entry.name}", default_rank), category, entry)
            for category, entries in ordered_categories
            for entry in entries
        ]
        fallback_entries.sort(key=lambda item: (item[0], item[1].casefold(), item[2].name.casefold()))
        for _rank, _category, entry in fallback_entries:
            key = entry.name.casefold()
            if key not in seen:
                seen.add(key)
                result.append(entry.name)
                if len(", ".join(result)) >= _MIN_SKILLS_LINE_CHARS or len(result) >= _MAX_SKILLS:
                    break
    return tuple(result)
...
def _render_selection(
    ...
    if profile.education:
        ...
        for entry in profile.education:
            ...
            if selection.selected_coursework:
                cw_text = ", ".join(selection.selected_coursework)
                lines.append(r"\resumeItemListStart")
                lines.append(r"\resumeItem{\textbf{Relevant Coursework}: " + _latex_escape(cw_text) + r"}")
...
def _audit_rendered_resume(
    ...
    supported_terms = tuple(term for term in terms if _ats_contains(profile_evidence, term))
    unsupported_terms = tuple(term for term in terms if term not in supported_terms)
    supported_but_missed = tuple(term for term in rendered_missing if term in supported_terms)
    high_priority = tuple(term for term in terms if term_weights.get(term.casefold(), 0.0) >= 2.5)
    high_priority_missing = tuple(term for term in high_priority if term not in rendered_matches)
    supported_high_priority_missing = tuple(term for term in high_priority_missing if term in supported_terms)
    unsupported_inserted = tuple(term for term in rendered_matches if term in unsupported_terms)

    claim_texts = _profile_claim_texts(profile)
    section_claims: dict[str, list[dict[str, Any]]] = {name: [] for name, _body in plan.sections}
    for identifier, sources in plan.selected_claims:
        if identifier.startswith("edu_"):
            section = "Education"
        elif identifier.startswith("exp_"):
            section = "Experience"
        elif identifier.startswith("lead_"):
            section = "Leadership"
        elif identifier.startswith("proj_"):
            section = "Projects"
        elif identifier.startswith("skill:"):
            section = "Technical Skills"
        else:
            section = "Other"
        claim_text = claim_texts.get(identifier, "")
        relevance = tuple(term for term in terms if _ats_contains(claim_text, term))
        section_claims.setdefault(section, []).append(
            {
                "id": identifier,
                "sources": list(sources),
                "relevance_terms": list(relevance),
                "reason": (
                    "source-backed claim matches required terms"
                    if relevance
                    else "source-backed profile claim retained as section context"
                ),
            }
        )
    section_evidence = []
        ...
        section_evidence.append(
            {
                ...
                "source_ids": sorted({source for claim in claims for source in claim["sources"]}),
                "relevance_terms": sorted(
                    {term for claim in claims for term in claim["relevance_terms"]},
                    key=lambda value: (value.casefold(), value),
                ),
                "reason": (
                    "source-backed claims with matched job terms"
                    if any(claim["relevance_terms"] for claim in claims)
                    else "source-backed profile section context"
                ),
            }
        )

    repetition_findings: list[str] = []
    for term in rendered_matches:
        spaced_text, _compact_text = _ats_text_forms(rendered_text)
        spaced_term, _compact_term = _ats_term_forms(term)
        matches = list(
            re.finditer(
                r"(?<!\w)" + re.escape(spaced_term) + r"(?!\w)",
                spaced_text,
            )
        )
        if len(matches) <= 4:
            continue
        for start, end in zip(matches, matches[4:]):
            if end.start() - start.start() <= max(80, len(spaced_term) * 8):
                repetition_findings.append(
                    f"{term} repeated densely {len(matches)} times"
                )
                break
    coverage = 1.0 if not terms else len(rendered_matches) / len(terms)
    return {
        "normalized_required_terms": sorted(
            {_ats_text_forms(term)[0] for term in terms if _ats_text_forms(term)[0]},
            key=lambda value: (value.casefold(), value),
        ),
        "rendered_matched_terms": list(rendered_matches),
        "rendered_missing_terms": list(rendered_missing),
        "rendered_coverage_ratio": coverage,
        "supported_terms": list(supported_terms),
        "supported_but_missed_terms": list(supported_but_missed),
        "unsupported_terms": list(unsupported_terms),
        "high_priority_terms": list(high_priority),
        "high_priority_missing_terms": list(high_priority_missing),
        "supported_high_priority_missing_terms": list(supported_high_priority_missing),
        "unsupported_inserted_terms": list(unsupported_inserted),
        "repetition_findings": repetition_findings,
        "section_evidence": section_evidence,
        "passed": not supported_high_priority_missing and not unsupported_inserted and not repetition_findings,
    }

def optimize_resume(profile: ResumeProfile, job: ResumeJob, advice: Any | None = None) -> ResumePlan:
    """Select source-backed claims, optionally using validated ordering hints."""
    ...
    claim_rank: Mapping[str, int] = advice.claim_rank if advice is not None else {}
    default_rank = len(claim_rank) + 1
    exp_scores: dict[str, float] = {}
    exp_bullet_scores: dict[tuple[str, str], float] = {}
    for entry in profile.experience:
        entry_score = _score_text_against_terms(_entry_text(entry), terms) + _advisory_bonus(entry.id, claim_rank)
        exp_scores[entry.id] = entry_score
        for bullet in entry.bullets:
            exp_bullet_scores[(entry.id, bullet.id)] = (
                _score_text_against_terms(_bullet_text(bullet), terms)
                + entry_score * 0.25
                + _advisory_bonus(bullet.id, claim_rank)
            )
    ...
    exp_order = sorted(
        range(len(profile.experience)),
        key=lambda i: (
            -_date_key(profile.experience[i].dates.start)[0],
            -_date_key(profile.experience[i].dates.start)[1],
            claim_rank.get(profile.experience[i].id, default_rank),
            i,
        ),
    )
    ...
    for index in exp_order:
        ...
        bullet_ids = tuple(
            ...
            for bullet in sorted(
                entry.bullets,
                key=lambda b: (
                    -exp_bullet_scores[(entry.id, b.id)],
                    claim_rank.get(b.id, default_rank),
                    entry.bullets.index(b),
                ),
            )
        )
    ...
    if exp_selection:
        primary = max(
            (entry_id for entry_id, _bullet_ids in exp_selection),
            key=lambda entry_id: (
                exp_scores[entry_id],
                -claim_rank.get(entry_id, default_rank),
                -next(i for i, pair in enumerate(exp_selection) if pair[0] == entry_id),
            ),
        )
    ...
    for entry in profile.leadership:
        score = _score_text_against_terms(_entry_text(entry), terms) + _advisory_bonus(entry.id, claim_rank)
        lead_scores[entry.id] = score
        for bullet in entry.bullets:
            lead_bullet_scores[(entry.id, bullet.id)] = (
                _score_text_against_terms(_bullet_text(bullet), terms)
                + score * 0.25
                + _advisory_bonus(bullet.id, claim_rank)
            )
    lead_candidates = [entry for entry in profile.leadership if lead_scores[entry.id] > 0]
    lead_candidates.sort(key=lambda entry: (-lead_scores[entry.id], claim_rank.get(entry.id, default_rank), -_date_key(entry.dates.start)[0], -_date_key(entry.dates.start)[1], entry.id))
    lead_selection = tuple(
        (
            entry.id,
            tuple(b.id for b in sorted(entry.bullets, key=lambda b: (-lead_bullet_scores[(entry.id, b.id)], claim_rank.get(b.id, default_rank), entry.bullets.index(b)))),
        )
    ...
    for entry in profile.projects:
        ...
        for bullet in entry.bullets:
            project_bullet_scores[(entry.id, bullet.id)] = (
                _score_text_against_terms(_bullet_text(bullet), terms)
                + score * 0.25
                + _advisory_bonus(bullet.id, claim_rank)
            )
    project_candidates = [entry for entry in profile.projects if entry.enabled and project_scores[entry.id] > 0]
    project_candidates.sort(key=lambda entry: (-project_scores[entry.id], claim_rank.get(entry.id, default_rank), -_date_key(entry.dates.start)[0], -_date_key(entry.dates.start)[1], entry.id))
    project_selection = tuple(
        (
            entry.id,
            tuple(
                b.id
                for b in sorted(entry.bullets, key=lambda b: (-project_bullet_scores[(entry.id, b.id)], claim_rank.get(b.id, default_rank), entry.bullets.index(b)))[:_MAX_PROJECT_BULLETS]
            ),
        )
    ...
    compressed_skills = _select_skills(profile, terms, claim_rank)
    ...
    return ResumePlan(
        ...
        coursework_order=tuple(advice.ranked_coursework) if advice is not None else (),
    )
...
def _fingerprint_inputs(
    ...
    skill_bytes: bytes,
    compiler_identity: str = "auto",
    advice_digest: str | None = None,
) -> str:
    digests = _input_digests(job, profile_bytes, template_bytes, skill_bytes, compiler_identity)
    advisory_mode = "deterministic"
    if (
        os.environ.get("RESUME_ADVISORY_ENABLED", "").strip().casefold() in {"1", "true", "yes"}
        and (os.environ.get("OLLAMA_CLOUD_API_KEY") or os.environ.get("OLLAMA_API_KEY"))
    ):
        advisory_mode = (
            os.environ.get("OLLAMA_CLOUD_MODEL")
...
def _find_compiler() -> tuple[str, str, tuple[int, int, int, int, int, int]]:
    """Return the first discoverable compiler that passes identity validation."""
    for candidate in ("tectonic", "pdflatex"):
        ...
        try:
            identity, signature = _compiler_identity(resolved)
        except RuntimeError:
            continue
        return resolved, identity, signature
    raise RuntimeError("no safe, successful LaTeX compiler found (tried tectonic, pdflatex)")
...
def _compiler_stat_signature(executable: str) -> tuple[int, int, int, int, int, int]:
    ...
    if not stat.S_ISREG(info.st_mode) or info.st_mode & 0o022:
        raise RuntimeError("resolved compiler executable must be a non-writable regular file")
    return (
        info.st_dev,
        info.st_ino,
...
def _compiler_identity(
    ...
    resolved = os.path.realpath(executable)
    compiler_signature = _compiler_stat_signature(executable)
    ...
    if _compiler_stat_signature(executable) != compiler_signature:
        raise RuntimeError("compiler executable changed during version probe")
    ...
    stat_payload = {
        "dev": compiler_signature[0],
        "ino": compiler_signature[1],
        "mode": compiler_signature[2],
        "size": compiler_signature[3],
        "mtime_ns": compiler_signature[4],
        "ctime_ns": compiler_signature[5],
    }
    ...
    return identity, compiler_signature
...
def _compile_latex(
    ...
    if _compiler_stat_signature(argv[0]) != compiler_signature:
        raise RuntimeError("compiler executable changed before invocation")
    executable = Path(argv[0]).name
    ...
    if _compiler_stat_signature(argv[0]) != compiler_signature:
        raise RuntimeError("compiler executable changed during invocation")
...
def _replace_plan_selection(plan: ResumePlan, profile: ResumeProfile, selection: _Selection) -> ResumePlan:
    ...
    return ResumePlan(
        ...
        coursework_order=plan.coursework_order,
    )
...
def _remove_coursework_content(selection: _Selection) -> _Selection | None:
    if not selection.selected_coursework:
        return None
    return _Selection(
...
def _remove_leadership_content(selection: _Selection) -> _Selection | None:
    if not selection.leadership:
        return None
    return _Selection(selection.experience, selection.leadership[:-1], selection.projects, selection.primary_experience, selection.selected_coursework)
...
def _remove_optional_experience(
    ...
    pairs = list(selection.experience)
    pairs.pop(index)
    return _Selection(tuple(pairs), selection.leadership, selection.projects, selection.primary_experience, selection.selected_coursework)
        ...
        if chosen is None:
            ...
            candidates.append(
                _replace_plan_selection(
                    plan,
                    profile,
                    _Selection(experience, selection.leadership, selection.projects, selection.primary_experience, selection.selected_coursework),
                )
            )
            continue
        for bullet_id in ordered:
            ...
            candidates.append(
                _replace_plan_selection(
                    plan,
                    profile,
                    _Selection(tuple(experience), selection.leadership, selection.projects, selection.primary_experience, selection.selected_coursework),
                )
            )
    ...
    for entry in sorted(
        ...
        candidates.append(
            _replace_plan_selection(
                plan,
                profile,
                _Selection(selection.experience, leadership, selection.projects, selection.primary_experience, selection.selected_coursework),
            )
        )
    ...
    for entry in sorted(
        ...
        candidates.append(
            _replace_plan_selection(
                plan,
                profile,
                _Selection(selection.experience, selection.leadership, projects, selection.primary_experience, selection.selected_coursework),
            )
        )
    if profile.education:
        all_coursework: list[str] = []
        for edu in profile.education:
            all_coursework.extend(edu.coursework)
        if all_coursework:
            coursework_rank = {value: index for index, value in enumerate(plan.coursework_order)}
            scored_cw = sorted(
                all_coursework,
                key=lambda cw: (
                    -_score_text_against_terms(cw, terms),
                    coursework_rank.get(cw, len(coursework_rank) + 1),
                    cw.casefold(),
                ),
            )
            for count in (1, min(2, len(scored_cw))):
                cw_candidate = tuple(scored_cw[:count])
                if cw_candidate != selection.selected_coursework:
                    candidates.append(
                        _replace_plan_selection(
                            plan,
                            profile,
                            _Selection(
                                ...
                                selection.projects,
                                selection.primary_experience,
                                selected_coursework=cw_candidate,
                            ),
                        )
                    )
...
def _expand_plan(
    ...
    def action_key(candidate: ResumePlan) -> tuple[str, tuple[str, ...]]:
        ...
        if candidate.selection and current.selection and candidate.selection.selected_coursework != current.selection.selected_coursework:
            return ("coursework", tuple(value.casefold() for value in candidate.selection.selected_coursework))
    ...
    def run_phase(skills_only: bool) -> bool:
        nonlocal current, current_audit, stage_matches_current, expansion_attempts
        while True:
            ...
            for candidate in _expansion_candidates(current, profile, job):
                ...
                attempted.add(key)
                pages, candidate_text = _compile_plan(
                    candidate, job, template_text, output_dir, compiler, compiler_signature
                )
                ...
                if candidate_audit["unsupported_inserted_terms"]:
                    continue
                current = candidate
                current_audit = candidate_audit
                found_candidate = True
                stage_matches_current = True
                break
    ...
    if not stage_matches_current:
        _compile_plan(current, job, template_text, output_dir, compiler, compiler_signature)
    return current
...
def _trim_plan(
    ...
    for _attempt in range(256):
        ...
        if selection is None:
            raise RuntimeError("cannot trim a plan without selection metadata")
        next_selection = _remove_coursework_content(selection)
...
_ATS_AUDIT_KEYS = frozenset(
    {
        "normalized_required_terms",
        "rendered_matched_terms",
        "rendered_missing_terms",
        "rendered_coverage_ratio",
        "supported_terms",
        "supported_but_missed_terms",
        "unsupported_terms",
        "high_priority_terms",
        "high_priority_missing_terms",
        "supported_high_priority_missing_terms",
        "unsupported_inserted_terms",
        "repetition_findings",
        "section_evidence",
        "passed",
    }
...
def _validate_ats_audit_payload(value: Any) -> None:
    if not isinstance(value, dict):
        raise RuntimeError("ATS audit must be an object")
    try:
        _check_exact_keys(value, _ATS_AUDIT_KEYS, "ATS audit")
    except ValueError as exc:
        raise RuntimeError(str(exc)) from exc
    list_keys = _ATS_AUDIT_KEYS - {"rendered_coverage_ratio", "section_evidence", "passed"}
    for key in list_keys:
        if not isinstance(value[key], list) or any(type(item) is not str for item in value[key]):
            raise RuntimeError(f"ATS audit field is invalid: {key}")
    if type(value["rendered_coverage_ratio"]) not in (int, float):
        raise RuntimeError("ATS audit coverage is invalid")
    if not 0.0 <= float(value["rendered_coverage_ratio"]) <= 1.0:
        raise RuntimeError("ATS audit coverage is out of range")
    if type(value["passed"]) is not bool:
        raise RuntimeError("ATS audit pass flag is invalid")
    if not isinstance(value["section_evidence"], list):
        raise RuntimeError("ATS audit section evidence is invalid")
...
def _build_report(
    ...
    skill_sha256: str,
    advice_digest: str | None = None,
    advice_status: str = "not_requested",
    ats_audit: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if ats_audit is None:
        raise RuntimeError("ATS audit is required before publishing a report")
    return {
        ...
        "advisory_status": advice_status,
        "advisory_used": advice_status == "used",
        "advice_digest": advice_digest,
        "ats_audit": ats_audit,
    }
...
def _validate_manifest_and_cache(
    ...
    try:
        ...
        raise RuntimeError(str(exc)) from exc
    _validate_ats_audit_payload(report["ats_audit"])
    if not report["ats_audit"]["passed"]:
        raise RuntimeError("cached report ATS audit did not pass")
    if (
...
def generate_resume(
    ...
    *,
    ...
    if compiler is None:
        resolved_compiler, compiler_identity, compiler_signature = _find_compiler()
    else:
        resolved_compiler = _resolve_compiler_path(compiler)
        compiler_identity, compiler_signature = _compiler_identity(resolved_compiler)
    advice = None
    advice_digest: str | None = None
    advice_status = "not_requested"
    advisory_requested = (
        os.environ.get("RESUME_ADVISORY_ENABLED", "").strip().casefold() in {"1", "true", "yes"}
        and bool(os.environ.get("OLLAMA_CLOUD_API_KEY") or os.environ.get("OLLAMA_API_KEY"))
    )
    if advisory_requested:
        from .advisor import request_resume_advice

        advice = request_resume_advice(profile, job)
        if advice is None:
            advice_status = "failed"
            advice_digest = _sha256_hex(b"advisory-failed")
        else:
            advice_status = "used"
            advice_digest = _sha256_hex(
                _canonical_json(
                    {
                        "ranked_claim_ids": list(advice.ranked_claim_ids),
                        "ranked_coursework": list(advice.ranked_coursework),
                    }
                )
            )
    ...
    try:
        ...
        plan = optimize_resume(profile, job, advice=advice)
        ...
        if not ats_audit["passed"]:
            raise RuntimeError("generated resume failed ATS evidence audit")
